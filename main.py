...
##### BOT VERSION
botversion = "2.5"
updateNotice = "Обновлена логика; добавлены приколы в /grow; добавлены уведомления об обновлении бота, потому что никто не проверяет /ver 😭; про остальное я забыла, потому что это было написано где-то две недели назад..."
##### BOT VERSION
...
responseList = ["Отхлебнув зелья, Вы чувствуете изменения внутри себя.", "Воткнув в себя случайный шприц, Вы чувствуете изменения внутри себя.", "Съев странную вещь, Вы чувствуете изменения внутри себя.", "Обняв случайного Фокса, Вы чувствуете изменения внутри себя.", "Зайдя не в ту подворотню... Не важно. Вы чувствуете изменения внутри себя.", "Съев какую-то таблетку с пола, Вы чувствуете изменения внутри себя.", "Заснув не в том вместе, Вы чувствуете изменения внутри себя."]
...
def grow(update: Update, context: CallbackContext):
	...
	ach10 | Уу... Оу май~ — дорасти до 690 см
	...
	if randnum == 0:
		...
		update.message.reply_text(f"{random.choice(responseList)} Вы стали короче на: {randnum*-1} см.\nТеперь Ваш текущий рост: {totalHeight} см!\nСледующая попытка будет доступна через 6 часов.")
		...
		update.message.reply_text(f"{random.choice(responseList)} Вы выросли на: {randnum} см.\nТеперь Ваш текущий рост: {totalHeight} см!\nСледующая попытка будет доступна через 6 часов.")
